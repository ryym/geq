...
func buildBuilderFileDef(pkg *packages.Package) (def *builderFileDef, err error) {
	...
	rowId2Mapper := make(map[string]*tableDef)
	...
	for i := 0; i < nTables; i++ {
		...
		rowId2Mapper[fieldType.String()] = &td
	...
	for i := 0; i < relStruct.NumFields(); i++ {
		...
		if !ok {
			return nil, fmt.Errorf("type of field of GeqRelationships %s must be unnamed struct", field.Name())
		}
		for j := 0; j < fieldStruct.NumFields(); j++ {
			f := fieldStruct.Field(j)
			fType := f.Type().String()
			mapperR, ok := rowId2Mapper[fType]
			if !ok {
				return nil, fmt.Errorf("invalid relationship type: %s", fType)
			}
			tag := reflect.StructTag(fieldStruct.Tag(j))
			relDef := tag.Get("geq")
			if relDef == "" {
				return nil, fmt.Errorf("relationship of must be defined in tag")
			}
			...
			for i, t := range tables {
				...
				case rs.MapperL:
					...
					tables[i].Relships = append(tables[i].Relships, rs)
				case rs.MapperR:
					for _, f := range t.Fields {
						if f.Name == rs.FieldR {
							ftR = f.Type
	...
	imports["github.com/ryym/geq"] = struct{}{}
	def = &builderFileDef{
...
type Table{{.Name}} struct {
	*geq.TableBase
	...
	{{end -}}
	{{range .Relships -}}
	{{.RelName}} *geq.Relship[{{.RowR}}, {{.FieldType}}]
	{{end -}}
}
